...
from models.on_manifold_drl.v4_joint import MPDR_Joint
from models.on_manifold_drl.v5_single import MPDR_Single, MPDR_Ensemble
...
class MPDRTrainer(BaseTrainer):
    def train(self, model, opt, d_dataloaders, logger=None, logdir='', scheduler=None, clip_grad=None):
        ...
        # option for learning on normalized data from pretrained backbones
        if cfg.get('switch_spherical', False):
            model.ae.out_spherical = True
            print('Autoencoder output is switched to spherical')
        ...
        if cfg.get('init_net_x_ae', None) is not None:
            # load net_x from pretrained ae
            state_dict = torch.load(cfg['init_net_x_ae'], map_location=self.device)['model_state']
            # state_dict = {k.replace('ae.', ''): v for k, v in state_dict.items() if k.startswith('ae.')}

            encoder_state = {k.replace('ae.encoder.','') : v for k, v in state_dict.items() if k.startswith('ae.encoder')}
            decoder_state = {k.replace('ae.decoder.','') : v for k, v in state_dict.items() if k.startswith('ae.decoder')}
            model.net_x.encoder.load_state_dict(encoder_state)
            model.net_x.decoder.load_state_dict(decoder_state)
            n_netx_epoch = 0
            print(f'net x loaded {cfg["init_net_x_ae"]}')
        ...
        for i_epoch in tqdm(range(n_nae_epoch), bar_format='{l_bar}{bar:20}{r_bar}'):
            for x, _ in tqdm(indist_train_loader, bar_format='{l_bar}{bar:20}{r_bar}'):
                ...
                if i % cfg.val_interval_nae == 1 and i != 1:
                    '''AUC'''
                    d_result = self.get_auc(model, d_dataloaders)
                    # self.save_model_ompd(model, mpdr_opt, logdir, i_iter=i, i_epoch=i_epoch, d_result=d_result, record_iter=True)
    ...
    def load_ae(self, model):
        ...
        if isinstance(model, MPDR_Ensemble):
            for i, ckpt in enumerate(cfg['load_ae']):
                state_dict = torch.load(ckpt)['model_state']
                encoder_state = {k.replace('ae.encoder.','') : v for k, v in state_dict.items() if k.startswith('ae.encoder')}
                decoder_state = {k.replace('ae.decoder.','') : v for k, v in state_dict.items() if k.startswith('ae.decoder')}
                # ae_state_dict = {k.replace('ae.', ''):v for k, v in state_dict.items() if k.startswith('ae')}
                model.l_ae[i].encoder.load_state_dict(encoder_state)
                model.l_ae[i].decoder.load_state_dict(decoder_state)
            return
    ...
    def get_opt(self, model):
        ...
        if isinstance(model, MPDR):
            ...
            ae_opt = None
            if cfg.get('opt', None) == 'fix_D':
                mpdr_opt = Adam(list(model.net_x.encoder.parameters()) + [model.net_x.out_scale], lr=cfg.nae_lr)
            else:
                mpdr_opt = Adam(model.net_x.parameters(), lr=cfg.nae_lr)

            netx_opt = Adam(list(model.net_x.parameters()), lr=cfg.get('netx_lr', 1e-4))
            ...
            if cfg.get('ae_opt', 'adam') == 'adam':
                if len(list(model.ae.parameters())) > 0:
                    ae_opt = Adam(list(model.ae.parameters()), lr=cfg.ae_lr)
                else:
                    ae_opt = None
            else:
                ae_opt = SGD(model.ae.parameters(), lr=cfg.ae_lr)
            ...
            if cfg.get('opt', None) == 'fix_D':  # fix decoder
                ...
                mpdr_opt = Adam(list(model.net_x.encoder.parameters()), lr=cfg.nae_lr)
            else:
    ...
    def save_model_ompd(self, model, opt, logdir, i_iter, i_epoch, record_iter=False, **kwargs):
        state = {'model_state': model.state_dict(), 'opt_state': opt.state_dict(),
                 'i': i_iter, 'i_epoch': i_epoch, **kwargs}
        if record_iter:
            fname = f'mpdr_iter_{i_iter}.pkl'
        else:
